...
module.exports = {
    ...
    args: true,
    usage: "<email address>` OR `code <code>",
    guildOnly: false,
    hideHelp: false,

    getMeets(guild, UID, email) {
        // Array of dates
        let discordMeets = [];
        let meetMeets = [];
        
        let discordLogs = handler.getGuildValue("meetings", guild);

        Object.keys(discordLogs).forEach(id => {
            if (discordLogs[id]["users"].hasOwnProperty(UID) && discordLogs[id]["users"][UID].valid) {
                let date = SnowflakeUtil.deconstruct(id).date;
                ...
                discordMeets.push([year, month, day].join("-"));
            }
        });

        meetMeets = handler.getMeets(guild.id, email);
        
        return meetMeets.concat(discordMeets);
    ...
    async execute(message, args) {
        ...
        if (args[0].includes('@')) {
            ...
            if (handler.getUser(message.guild.id, message.author.id)) {
...
`;
            ...
            if (handler.getUser(guild.id, message.author.id)) {
                message.reply("You've already done this! You can't do it multiple times :laughing:");
                return;
            }
            ...
            let oldnum = handler.getAttendance(guild.id, message.author.id).length;
